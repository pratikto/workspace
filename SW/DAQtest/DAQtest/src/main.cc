...
int main(){
	...
	while(true){
		...
		if(READY_0_trig){
			...
			Xil_Out32(XPAR_PS7_DDR_0_S_AXI_BASEADDR + (4*Index0 + 0), *(baseaddr_DAQ+1));
		...
		if(READY_1_trig){
			...
			Xil_Out32(XPAR_PS7_DDR_0_S_AXI_BASEADDR + (4*Index1 + 2), *(baseaddr_DAQ+3));
			Xil_Out32(XPAR_PS7_DDR_0_S_AXI_BASEADDR + (4*Index1 + 3), *(baseaddr_DAQ+2));
    	...
    	if (!checkBit(*(baseaddr_DAQ+4), 5) and writeLog){
    		...
    		for(int i=0; i<Index0; i++){
    			...
    			sprintf(Buffer_logger, "%u,%u,%u,,,,\n",
    					i,
						Xil_In32(XPAR_PS7_DDR_0_S_AXI_BASEADDR + (4*i + 0)),
						Xil_In32(XPAR_PS7_DDR_0_S_AXI_BASEADDR + (4*i + 1))
    			//							counter0[Index0].high,
    			//							counter0[Index0].low
    			);
    			print(Buffer_logger);
    		...
    		for(int j=0; j<Index1; j++){
    			...
    			sprintf(Buffer_logger, ",,,,%u,%u,%u,\n",
    					j,
						Xil_In32(XPAR_PS7_DDR_0_S_AXI_BASEADDR + (4*j + 2)),
						Xil_In32(XPAR_PS7_DDR_0_S_AXI_BASEADDR + (4*j + 3))
						//							counter0[Index0].high,
						//							counter0[Index0].low
    			);
    			print(Buffer_logger);
...
//
//    		// Creating new file with read/write permissions
//    		Log_File1 = (char *)FileName1;
//    		result = f_open(&file1, Log_File1, FA_CREATE_ALWAYS | FA_WRITE | FA_READ);
//    		if (result!= 0) {
//    			print("failed creating new files\n\r");
//    			return XST_FAILURE;
//    		}
...
//
//    		for(int j=0; j<Index1; j++){
//        		sprintf(Buffer_logger, "iter-%d!\n", j);
//    			print(Buffer_logger);
//
//    			sprintf(Buffer_logger, "%u,%u,%u\n",
//    					i,
//						Xil_In32(XPAR_PS7_DDR_0_S_AXI_BASEADDR + (4*i + 2)),
//						Xil_In32(XPAR_PS7_DDR_0_S_AXI_BASEADDR + (4*i + 3))
//    			//							counter0[Index0].high,
//    			//							counter0[Index0].low
//    			);
//    			print(Buffer_logger);
...
//    		}
//
//    		print("Done writing counter 1 data to SD card\n\r");
